...
class list_eslable(list_parent_class):
    ...
    def create_list(self):
        ...
        self.previously_esl_flagged_exists = os.path.exists(PREVIOUSLY_ESL_FLAGGED_JSON)
        ...
        self.flagged:set = set(self.get_data_from_file(ESL_FLAGGED_JSON, list))
        self.blacklist_list:set = set(self.get_data_from_file(BLACKLIST_JSON, list))
        self.cell_changed:set = set(self.get_data_from_file(CELL_CHANGED_JSON, list))
    ...
    def check_previously_esl_flagged(self, esl_flagged:list[str]=None, full_warning:bool = True):
        self.blockSignals(True)
        if os.path.exists(PREVIOUSLY_ESL_FLAGGED_JSON) or esl_flagged is not None:
            try:
                selected = set()
                if esl_flagged is None:
                    with open(PREVIOUSLY_ESL_FLAGGED_JSON, 'r', encoding='utf-8') as f:
                        esl_flagged = json.load(f)
                        f.close()
                esl_flagged_set = set(esl_flagged)
                for row in range(self.rowCount()):
                    if not self.isRowHidden(row) and self.item(row, self.MOD_COL).checkState() == Qt.CheckState.Unchecked and self.item(row, self.MOD_COL).text() in esl_flagged_set:
                        self.item(row, self.MOD_COL).setCheckState(Qt.CheckState.Checked)
                        selected.add(self.item(row, self.MOD_COL).text())
                diff = esl_flagged_set - selected
                ...
                if diff and full_warning:
                    ...
                    missing_text = self.tr("The following plugins cannot be re-ESL flagged because they have dependents that are missing "\
                    ...
                    not_valid_text = self.tr("The following plugins cannot be re-ESL flagged because they no longer meet the conditions "\
                                            "(maybe new records exceed max count or it is now filtered out from settings or it is already ESL):") + "\n"
                    not_found_text = self.tr("The following plugins cannot be re-ESL flagged because they no-longer exist (this should be okay "\
                    ...
                    text = self.tr("Could not select the following files either due to filters, the files not existing, or the file is already flagged ESL:") +"\n"
                    for mod in diff:
                        text += "- " + mod + "\n"
                    write_to_file(text)
                    QMessageBox.warning(None, self.tr("Warning"), text, QMessageBox.StandardButton.Ok)
