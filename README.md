...
## BSArch.exe
ESLifier includes a modified BSArch.exe (from [TES5Edit](https://github.com/TES5Edit/TES5Edit/tree/master/Tools/BSArchive)) to extract pex, seq, facetint, facegeom, and voice files. The modified .dpr file is included in the repo [here](https://github.com/MaskPlague/ESLifier/blob/main/bsarch/BSArch.dpr). The modification allows for file filtering via the CLI.

## User Manual
Notes:
- If you use MO2 do $${\color{red}NOT}$$ add ESLifier.exe as an executable. Launching ESLifier through MO2 significantly slows down the file scanning process. It may also conflict with certain features. Instead set your mod manager mode to MO2 in the settings of ESLifier. Also, ESLifier can scan the Overwrite folder however, you will probably want to move everything in it to a mod instead, so that ESLifier's output can win any file conflicts and you won't keep the only copy of any configs generated in overwrite patched to ESLifier Output as your only copy.
...
On the first launch or when the paths are not set, you will be redirected to the settings page. Set all available paths to the required folderd. You will not be able to exit the settings page until they are set. The settings available are by default in the recommended configuration that should ensure the fewest issues when compacting or ESL flagging. You may want to disable _Show plugins with new CELL records_ if you think you may often install mods that will patch your existing mods with CELLs (the Patch New Page will warn you if you do install mods that do so later).
...
- _Show Plugins with new CELL records_ affects the scanning and display of both lists. It is on by default. If enabled, it will scan for and display mods that have new CELL records. If a new CELL record is defined in an ESL flagged plugin and then edited by another plugin then it can break the new CELL (Fixed by SSE Engine Fixes v7+ or the  _Generate Cell Master_ setting). This setting is used in combination with the setting _Hide ESM plugins with new CELL records that are overwriten_.
...
- _Hide plugins with new interior CELL records_ hides plugins that have new interior CELLs as they can have issues with not reloading properly on save load if the player doesn't restart the game (Fixed by SSE Engine Fixes v7+). This is off by default. The user should decide if they think these kinds of plugins are safe to ESL.
- _Hide plugins with new WRLD (worldspace) records_ hides plugins that have a new WRLD record as they lose landscape data when ESL flagged (the ground will disappear in the new worldspace this can be fixed by enabling _Generate Cell Master_).
- _Hide plugins with new WTHR (weather) records_ hides plugins that have a new WTHR record as they can be referenced by ENB files that are not patched by ESLifier.
- _Hide plugins that have SEQ files_ hides plugins that have a SEQ file as ESL flagging them may cause dialogue from them to not work until you save and reload in game.
- _Hide plugins that have PEX files with GetModByName_ hides plugins that that call the papyrus function "GetModByName" on them as that function only works properly on non-ESL plugins and the alternative "GetLightModByName" cannot simply be patched in as it would mess with the logic of the scripts.
...
- _Generate Cell Master_ will create a new plugin called ESLifier_Cell_Master.esm which is used to define new cells and worldspaces in ESLifier ESL flagged/compacted plugins. This circumvents the ESL + ESM cell bug and the ESL worldspace bug. This is disabled by default since all the related bugs except the WorldSpace ESL bug have been fixed by SSE Engine Fixes v7+. It also disables the Interior CEll Record setting and the WorldSpace setting.
- _Hide left/right list columns visually_ simply visually hides the relevant column in the relevant list.

# Documentation
## How to Build
Fork this project and install _Conda_. Then run ready_environment.py either in a terminal or your IDE. After that completes you can run 'conda activate eslifier\_env' to activate the environment.
Run "build\_ESLifier\_EXE.py" either via a terminal opened in the _ESLifier_ directory or via your IDE in the _eslifier\_env_ Conda environment.
